...
ds = xarray.open_zarr(asset.href,storage_options=asset.extra_fields["xarray:storage_options"],**asset.extra_fields["xarray:open_kwargs"])

import numpy as np
import datetime
import os
def compute_coefficients(pstar: float, pressure: xarray.DataArray) -> xarray.Dataset:
    dlev,dloglev = pstar - pressure, np.log(pstar) - np.log(pressure)
    ...
    p_left,p_right  = dloglev.where(is_left).sum("bottom_top"), -dloglev.where(is_right).sum("bottom_top")

    denominator = p_left + p_right
    denominator = denominator.where(denominator > 1e-6, 1e-6)
    factor = p_left / denominator
    return xarray.Dataset(
        {"is_left": is_left,"is_right": is_right,"p_left": p_left, "p_right": p_right,"denominator": denominator, "factor": factor}
    )

def interp2pressure(xda, cds):
    # Interpolate xda to pressure level used to compute coefficients in ``cds``
    xda_left,xda_right = xda.where(cds["is_left"]).sum("bottom_top"),xda.where(cds["is_right"]).sum("bottom_top")
    result = xda_left + (xda_right - xda_left) * cds["factor"]
    mask = (cds["is_right"].sum("bottom_top") > 0) & (cds["is_left"].sum("bottom_top") > 0)
